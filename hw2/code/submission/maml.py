...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):
        ...
        accuracies.append(util.score(features,labels))
        ...
        for _ in range (self._num_inner_steps):
            ...
            for (k,p),g in zip(parameters.items(),gradients):
                parameters[k] = parameters[k] - self._inner_lrs[k]*g
    ...
    def _outer_step(self, task_batch, train):
        ...
        for task in task_batch:
            ...
            query_embeddings = self._forward(images_query,params)
            o_loss = F.cross_entropy(query_embeddings,labels_query)
            outer_loss_batch.append(o_loss)
            accuracies_support_batch.append(accs)
            accuracy_query_batch.append(util.score(query_embeddings,labels_query))
    ...
    if not args.test:
        ...
        if not args.exp:
            ...
            for k in K:
                ...
                report[k]=maml.test(dataloader_test)
            with open(os.path.join(args.log_dir,'maml_exp.pkl'),'wb') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp',action='store_true',default=False,help='run experiment')
